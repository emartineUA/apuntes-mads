...
```
$ docker build -t emartineUA/prueba .
```

En el comando se define el nombre de la imagen
(`emartineUA/prueba`) y el directorio local en el que se va
...
```
$ docker run --rm emartineUA/mads-todolist:latest --spring.profiles.active=postgres-prod --POSTGRES_HOST=postgres
```
